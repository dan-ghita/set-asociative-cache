...
namespace SetAssociativeCache.Test
{
    public class LRUAssocaitiveCacheUnitTests : AssociativeCacheUnitTests
    {
        ...
        public void Add_CacheIsFull_RemovesLeastRecentlyUsedElements()
        {
            ...
            {
                m_cache.Add(new BitArray(m_cacheSize + i), (m_cacheSize + i).ToString());
                Assert.Equal(null, m_cache.Get(new BitArray(removalOrder[i])));
                Assert.Equal((m_cacheSize + i).ToString(), m_cache.Get(new BitArray(m_cacheSize + i)));
